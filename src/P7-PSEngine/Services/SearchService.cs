...
namespace P7_PSEngine.Services
{
    ...
    public class SearchService : ISearchService
    {
        ...
        {
            List<FileInformation> documents = await _db.FileInformation.Include(p => p.IndexInformations.Where(p => search.Contains(p.Word))).Where(p => p.IndexInformations.Any(index => search.Contains(index.Word))).AsNoTracking().ToListAsync();
